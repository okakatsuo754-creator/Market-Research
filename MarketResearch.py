...
# 列構成: 国名, カテゴリ, 取引種別, アイテム名, 価格, 備考
headers_list = ["国名", "カテゴリ", "取引種別", "アイテム名", "価格", "備考"]

if len(raw_data) > 1:
    df = pd.DataFrame(raw_data[1:], columns=raw_data[0])
    # もし古いシートに「取引種別」がない場合、自動で「販売」として埋める
...
with tab1:
    ...
    if not df.empty:
        ...
        # 選択したアイテムの販売と買取を並べて表示
        compare_df = df[df["アイテム名"] == target_item].sort_values("価格")
        
        col_chart1, col_chart2 = st.columns(2)
        with col_chart1:
            st.write(f"🛒 **{target_item} の販売価格**")
            sell_df = compare_df[compare_df["取引種別"] == "販売"]
            if not sell_df.empty:
                st.bar_chart(sell_df.set_index("国名")["価格"])
            else:
                st.info("販売データなし")

        with col_chart2:
            st.write(f"💰 **{target_item} の買取価格**")
            buy_df = compare_df[compare_df["取引種別"] == "買取"]
            if not buy_df.empty:
                st.bar_chart(buy_df.set_index("国名")["価格"])
            else:
                st.info("買取データなし")